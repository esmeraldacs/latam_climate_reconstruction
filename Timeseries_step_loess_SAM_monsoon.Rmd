...
```{r}
sam_raster <- raster::raster("Output_data/summer_precip_percentage.tif") %>%
  raster::rasterToPoints() %>%
  as.data.frame(stringsAsFactors = FALSE) %>%
  rename(lon = x, lat = y, precip_percentage = summer_precip_percentage)
...
```{r}
sam_polygon <- sf::st_read("Input/Precipitation_polygon/10_levels_contour_level_29.7.shp") #%>% sf::st_make_valid()

# Simplify and make valid
sam_polygon <- ms_simplify(sam_polygon, keep = 0.99, keep_shapes = TRUE) %>% st_make_valid()
...
ggplot() +
  geom_raster(data = sam_raster, aes(x = lon, y = lat, fill = precip_percentage)) +
  scico::scale_fill_scico(palette = 'lapaz',direction = -1) +
  geom_sf(data = sam_polygon, fill="transparent",color = "gray30", size = 1.5 ) +
...
latam <- read.csv("C:/Users/esmer/UoR/latinam_climate/Output_train_amg/Train_amg_reconstruction_output_Hill_15.2.csv") |>
 ...
 # filter(!entity  %in% c("CURACAB","CURACAA")) |>
  filter(elvation > -100) |>
  mutate(entity = paste0(entity,"_",latitude,"_",longitude,"_",elvation)) |>
  filter(age <= 11700)
  

latam_points_sf <- latam |> sf::st_as_sf(coords = c("longitude", "latitude"), crs = 4326)
```



```{r}
filtered_points <- sf::st_join(latam_points_sf, sam_polygon, join = sf::st_within) %>% filter(!is.na(level))

sam_pollen <- filtered_points |>
  mutate(latitude = map_dbl(geometry, ~sf::st_coordinates(.)[, "Y"]),
         longitude = map_dbl(geometry, ~sf::st_coordinates(.)[, "X"])) |>
  as.data.frame() |>
  select(-c(geometry, level, piece, group))


aux <- sam_pollen |> select(entity, latitude, longitude) |> distinct() |>
  mutate(id_false = row_number())


ggplot() +
  geom_point(data=latam, aes(x=longitude,y=latitude), shape = 23) +
  geom_sf(data = sam_polygon) +
  geom_point(data = sam_pollen, aes(x=longitude,y=latitude), colour="red") +
  ggrepel::geom_text_repel(data = aux, aes(x=longitude,y=latitude, label=id_false)) +
  theme_test() 
```
...
map1 <- ggplot() +
  ...
  scico::scale_fill_scico(palette = 'lapaz',direction = -1) +
  geom_sf(data = sam_polygon, fill="transparent",color = "gray30", size = 1.5 ) +
  geom_point(data = sam_pollen, aes(x=longitude,y=latitude), shape=21) +
  labs(title = "Percentage of Summer Precipitation",
       x = "Longitude",
       y = "Latitude",
       fill = "Precipitation (%)") +
...
site_metadadata <- clim0 |> select(entity,latitude,longitude,elevation) |> distinct()
```
...
```{r}
embs5 <- embs4 |>
  select(entity,latitude,longitude,elevation,age,modern_MGS_temp,palaeo_MGS_temp,alpha,modern_co2,paleo_co2,ID,id) |>
  drop_na() |>
...
```{r}
alpha <- embs5 |> 
  select(entity,latitude,longitude,age,ID,alpha,alpha_pred_corr)
...
value_anomaly <- pollens1 |>
  ...
  select(variable,entity,age,value,modern_value,value_anomaly)
...
single_envelope |>
  ...
  ggplot(aes(x=age)) +
  geom_ribbon(aes(ymin=`5%`,ymax=`95%`, fill=variable), alpha = 0.5) +
  geom_step(aes(y= anomaly_mean)) +
  geom_line(aes(y= value_smooth, colour=variable)) +
  facet_wrap(~variable, scales="free_y") +
  theme_test()
```
