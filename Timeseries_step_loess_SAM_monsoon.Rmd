...
```{r}
moon_sam_raster <- raster("Output_data/summer_precip_percentage.tif") %>%
  rasterToPoints() %>%
  as.data.frame(stringsAsFactors = FALSE) %>%
  rename(lon = x, lat = y, precip_percentage = summer_precip_percentage)

ggplot() +
  geom_raster(data = moon_sam_raster, aes(x = lon, y = lat, fill = precip_percentage)) +
...
```{r}
moon_sam_polygon <- sf::st_read("Input/Precipitation_polygon/15_levels_polygon_level_33.shp") %>% st_make_valid()

plot(moon_sam_polygon)
```
...
ggplot() +
  geom_raster(data = moon_sam_raster, aes(x = lon, y = lat, fill = precip_percentage)) +
  scico::scale_fill_scico(palette = 'lapaz',direction = -1) +
  geom_sf(data = moon_sam_polygon, fill="transparent",color = "gray30", size = 1.5 ) +
...
```{r}
filtered_points <- sf::st_join(latam_points_sf, moon_sam_polygon, join = sf::st_within) %>% filter(!is.na(level))

moon_sam_pollen <- filtered_points |>
...
```{r}
map1 <- ggplot() +
 # geom_point(data=latam, aes(x=longitude,y=latitude), colour="blue") +
  geom_sf(data = moon_sam_polygon) +
  geom_point(data=moon_sam_pollen, aes(x=longitude,y=latitude)) +
  theme_test() +
